...
- Поиск top N результатов через `two.imusic.fm`; YouTube используется только как резервный поиск.
- Inline-кнопки для выбора конкретного результата.
- Скачивание mp3 через `two.imusic.fm`.
...
- YouTube/`yt-dlp` больше не используется как основной downloader при включенном iMusic.
...
Источник `two.imusic.fm` использует только публичные прямые ссылки на mp3, найденные на странице поиска. Он не обходит авторизацию, защиту или платный доступ.
